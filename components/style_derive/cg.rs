...
pub fn ref_pattern<'a>(variant: &'a VariantInfo, prefix: &str) -> (TokenStream, Vec<BindingInfo<'a>>) {
    let mut v = variant.clone();
    v.bind_with(|_| BindStyle::Ref);
    v.bindings_mut()
        .iter_mut()
        .for_each(|b| b.binding = Ident::new(&format!("{}_{}", b.binding, prefix), Span::call_site()));
