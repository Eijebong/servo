...
impl FetchTaskTarget for IpcSender<FetchResponseMsg> {
    ...
    fn process_response_chunk(&mut self, chunk: Vec<u8>) {
        debug!("process_response_chunk: {:?}", chunk);
        let _ = self.send(FetchResponseMsg::ProcessResponseChunk(chunk));
    }

    fn process_response_eof(&mut self, response: &Response) {
        debug!("process_request_eof: {:?}", response);
...
impl NetworkError {
    ...
    pub fn from_http_error(url: &ServoUrl, error: &HttpError) -> Self {
        NetworkError::Internal(error.description().to_owned())
    }
}
