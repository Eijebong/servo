...
impl HeadersMethods for Headers {
    ...
    fn Append(&self, name: ByteString, value: ByteString) -> ErrorResult {
        ...
        self.header_list.borrow_mut().insert(HeaderName::from_str(&valid_name).unwrap(), HeaderValue::from_bytes(&combined_value).unwrap());
        Ok(())
    }
    ...
    fn Set(&self, name: ByteString, value: ByteString) -> Fallible<()> {
        ...
        self.header_list.borrow_mut().insert(HeaderName::from_str(&valid_name).unwrap(), HeaderValue::from_bytes(&valid_value).unwrap());
        Ok(())
    }
}
