...
use typed_headers::{ContentLength, ContentType, ContentEncoding, HeaderMapExt};
use url::Position;
...
impl XMLHttpRequestMethods for XMLHttpRequest {
    ...
    fn SetRequestHeader(&self, name: ByteString, value: ByteString) -> ErrorResult {
        ...
        let value = match headers.get(name_str).map(HeaderValue::as_bytes) {
            Some(raw) => {
                debug!("SetRequestHeader: old value = {:?}", raw);
                            ...
                            for param in mime.params() {
                                if param.0 == mime::CHARSET {
                                    if !param.1.as_str_repr().eq_ignore_ascii_case(encoding) {
                                        let new_params: Vec<(Name, Value)> = mime.params().filter(|p| p.0 != mime::CHARSET).map(|p| (p.0, p.1)).collect();
                                        let new_mime =
                                            format!("{}/{}; charset={}{}{}",
                                                    mime.type_().as_ref(),
                                                    mime.subtype().as_ref(),
    ...
    fn GetResponseXML(&self) -> Fallible<Option<DomRoot<Document>>> {
        ...
        error!("{:?}", self.response_type.get());
        error!("{:?}", self.ready_state.get());
        match self.response_type.get() {
...
impl XMLHttpRequest {
    ...
    fn dispatch_progress_event(&self, upload: bool, type_: Atom, loaded: u64, total: Option<u64>) {
        let (total_length, length_computable) = if self.response_headers.borrow().contains_key(header::CONTENT_ENCODING) {
            (0, false)
        } else {
    ...
    fn filter_response_headers(&self) -> HeaderMap {
        ...
        headers.typed_remove::<SetCookie>();
